...
async def ocr_extract_details(
    ...
    extracted_fields = {
        ...
        "email": "",
        "dbtOption": "link_new",  # you can map this to option1/2/3/4 in frontend if needed
        "signature": None,
        "consent": False,
...
def auto_detect_dbt_fields_on_page(page):
    ...
    lines = _get_lines_with_positions(page)
    coords = {}

    # Account No. _________________ in A/c Name _______________
    coords["accountNumber"] = _find_after_word_sequence(
        words, ["account", "no"]
    )
    coords["fullName"] = _find_after_word_sequence(
        words, ["a/c", "name"]
    )

    # Aadhaar number (line: "I submit my Aadhaar number and voluntarily ...")
    coords["aadhaarNumber"] = _find_after_word_sequence(
        words, ["aadhaar", "number"]
    )

    # Mobile / Email ("Mobile No.:", "Email:")
    coords["mobileNumber"] = _find_after_word_sequence(
        words, ["mobile", "no"]
    )
    coords["email"] = _find_after_word_sequence(
        words, ["email"]
    )

    # Scan full lines for bank line, options, signature etc.
    for line in lines:
        lower = line["text"].lower()
        cy = (line["y0"] + line["y1"]) / 2.0

        # Bank + branch line just under "The Branch Manager,"
        if "the branch manager" in lower:
            x = line["x0"] + 10
            y = line["y1"] + 20  # slightly below printed line
            coords["bank_line"] = (x, y)

        # "I am maintaining a Bank account No. ______"
        if "i am maintaining a bank account no" in lower:
            x = line["x0"] + (line["x1"] - line["x0"]) * 0.55
            coords["maintain_account"] = (x, cy)

        # Option 1 ("I wish to seed my account No. ...")
        if "i wish to seed my account" in lower:
            coords["option1_tick"] = (line["x0"] - 15, cy)
            x_acct = line["x0"] + (line["x1"] - line["x0"]) * 0.45
        ...
        if (
            ...
            coords["option2_tick"] = (line["x0"] - 15, cy)

        # Option 3
        if "i already have an account with another bank" in lower:
            coords["option3_tick"] = (line["x0"] - 15, cy)

        # Option 4
        if "i do not wish to seed my accounts" in lower:
            coords["option4_tick"] = (line["x0"] - 15, cy)

        # Signature line ("Yours faithfully")
        if "yours faithfully" in lower:
            x = line["x0"] + 100
            y = line["y1"] + 20
            coords["signature_name"] = (x, y)
...
async def fill_dbt_form(
    ...
    # ----- 5) Compose combined fields -----
    bank_line = f"{branchName}, {bankName}" if branchName else bankName
    aadhaar_text = aadhaarNumber

    # ----- 6) Resolve coordinates (auto-detected → fallback to tuned defaults) -----

    # Fallbacks are tuned for your DBT_form_A4.pdf
    bank_x, bank_y = get_coord("bank_line", (160, 600))

    acct_x, acct_y = get_coord("accountNumber", (160, 560))
    name_x, name_y = get_coord("fullName", (400, 560))

    maintain_x, maintain_y = get_coord("maintain_account", (260, 520))

    aad_x, aad_y = get_coord("aadhaarNumber", (270, 480))

    mob_x, mob_y = get_coord("mobileNumber", (160, 205))
    email_x, email_y = get_coord("email", (160, 180))

    opt1_tick_x, opt1_tick_y = get_coord("option1_tick", (50, 438))
    opt2_tick_x, opt2_tick_y = get_coord("option2_tick", (50, 405))
    opt3_tick_x, opt3_tick_y = get_coord("option3_tick", (50, 372))
    opt4_tick_x, opt4_tick_y = get_coord("option4_tick", (50, 338))

    opt1_acct_x, opt1_acct_y = get_coord("option1_account", (270, 438))

    sig_x, sig_y = get_coord("signature_name", (160, 255))

    # ----- 7) Place text using these coordinates -----

    # 🏦 Bank & Branch (top section)
    draw_text(bank_x, bank_y, bank_line, max_chars=45)

    # 🧾 First line: "Account No. ______ in A/c Name ______"
    draw_text(acct_x, acct_y, accountNumber, max_chars=26)   # Account No.
    draw_text(name_x, name_y, fullName, max_chars=30)        # A/c Name

    # 💳 "I am maintaining a Bank account No. ______"
    draw_text(maintain_x, maintain_y, accountNumber, max_chars=26)

    # 🆔 "I submit my Aadhaar number ______"
    draw_text(aad_x, aad_y, aadhaar_text, max_chars=26)

    # ===========================
    #         TICK OPTIONS
    # ===========================

    # Respect dbtOption if frontend sends "option1"/"option2"/"option3"/"option4"
    if dbtOption == "option1":
        page.insert_text((opt1_tick_x, opt1_tick_y), "✓", fontsize=12, color=(0, 0, 0))
    elif dbtOption == "option2":
        page.insert_text((opt2_tick_x, opt2_tick_y), "✓", fontsize=12, color=(0, 0, 0))
    elif dbtOption == "option3":
        page.insert_text((opt3_tick_x, opt3_tick_y), "✓", fontsize=12, color=(0, 0, 0))
    elif dbtOption == "option4":
        page.insert_text((opt4_tick_x, opt4_tick_y), "✓", fontsize=12, color=(0, 0, 0))
    else:
        # Fallback: tick option1 by default
        page.insert_text((opt1_tick_x, opt1_tick_y), "✓", fontsize=12, color=(0, 0, 0))

    # Write Account Number in Option 1 line
    draw_text(opt1_acct_x, opt1_acct_y, accountNumber, max_chars=26)

    # ✍ Bottom: "Yours faithfully" – name
    draw_text(sig_x, sig_y, fullName, max_chars=30)

    # 📱 Mobile no.
    draw_text(mob_x, mob_y, mobileNumber, max_chars=15)

    # 📧 Email
    draw_text(email_x, email_y, email, max_chars=35)

    # ----- 8) Save and return file -----
